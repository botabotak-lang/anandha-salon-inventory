...
export function KartePage() {
    ...
    (async () => {
      try {
        const [cu, pr] = await Promise.all([
          api<{ customers: MasterCustomer[] }>("/api/customers"),
          api<{ products: { id: string; name: string; active?: boolean }[] }>("/api/products"),
        ]);
        setCustomers(cu.customers.filter((c) => c.active));
        setActiveProducts(pr.products.filter((p) => (p as { active?: boolean }).active !== false));
      } catch {
        setErr("マスタの取得に失敗しました");
        setCustomers([]);
      }
    })();
  }, []);
  ...
  useEffect(() => {
    ...
    setErr(null);
    loadSummary(selectedCustomerId.trim())
      .then(() => setErr(null))
      .catch((e) => {
        setSummary(null);
        setErr(e instanceof Error ? e.message : "カルテの取得に失敗しました");
      });
  }, [selectedCustomerId, loadSummary]);
  ...
  async function submitHandout(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedCustomerId.trim() || !summary) return;
    setMsg(null);
    setErr(null);
    ...
    try {
      ...
      await loadSummary(selectedCustomerId.trim());
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: "0.95rem", color: "#444", marginTop: "-0.25rem" }}>
        来店のたびに、渡した栄養剤の数量を入力して<strong>在庫を減らします</strong>。お客様は<strong>マスタに登録済み</strong>の方から選んでください。
      </p>
      <p style={{ fontSize: "0.85rem", marginBottom: "0.75rem" }}>
        <Link to="/customers">お客様マスタを編集する</Link>
      </p>
      ...
      <div className="card stack">
        ...
        <div>
          <label htmlFor="karte-customer">お客様</label>
          <select
            id="karte-customer"
            ...
            style={{ width: "100%", maxWidth: "28rem", fontSize: "1rem", padding: "0.5rem" }}
          >
        ...
        {customers.length === 0 && (
          <p style={{ fontSize: "0.9rem", color: "#666" }}>
            有効なお客様がいません。<Link to="/customers">お客様マスタ</Link>で登録してください。
          </p>
        )}
      </div>

      {selectedCustomerId.trim() && summary && (
        <>
      ...
      {selectedCustomerId.trim() && !summary && !err && (
